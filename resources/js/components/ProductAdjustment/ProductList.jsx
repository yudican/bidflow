        ...
        if (dataIndex === "stock") {
            return (
                <td>
                    <Input
                        disabled={true}
                        value={record[dataIndex]} // Tidak diformat saat diketik untuk menjaga "-" jika ada
                        style={{ marginBottom: 17 }}
                        onChange={(event) => {
                            let value = event.target.value;

                            // Izinkan hanya angka dan tanda minus
                            value = value.replace(/[^0-9-]/g, "");

                            // Pastikan hanya satu minus dan di posisi awal
                            if (value.indexOf("-") > 0) {
                                value = value.replace("-", ""); // Hapus minus yang tidak di awal
                            }

                            // Kirim nilai yang diubah ke parent
                            onChange({
                                value, // Kirim string langsung (termasuk "-") agar pengguna bisa mengetik
                                dataIndex,
                                key: record.key,
                            });
                        }}
                        onBlur={() => {
                            let finalValue = record[dataIndex];

                            // Ubah "-" menjadi 0 saat kehilangan fokus
                            if (finalValue === "-") {
                                finalValue = "0";
                            }

                            handleChange({
                                value: parseInt(finalValue, 10) || 0, // Pastikan nilai adalah angka valid
                                dataIndex,
                                key: record.key,
                            });
                        }}
