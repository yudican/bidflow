...
const InventoryProductStock = ({ type = "received" }) => {
  ...
  const show =
    ...
    inArray(type, ["transfer", "konsinyasi"])
  ...
  const menu = (
    <Menu>
      ...
      <Menu.Item>
        ...
        >
          {loadingExport ? <Spin /> : <DownloadOutlined />}
          ...
          <Dropdown.Button
            ...
            overlay={
              <Menu itemIcon={<RightOutlined />}>
                ...
                {inArray(inventory_type, ["received"]) &&
                  !inArray(status, ["cancel"]) && (
                    <Popconfirm
  ...
  const konsinyasiColumns = inArray(type, ["konsinyasi"])
    ? inventoryTransferStockKonsinyasiColumns(currentPage, perpage)
    : inventoryTransferStockColumns
  const columns =
  ...
  return (
    <Layout
      ...
      href={"/inventory-new"}
      title={
        inArray(type, ["transfer"])
          ? "List Item Transfer"
          : inArray(type, ["konsinyasi"])
          ? "List Item Transfer Konsinyasi"
          : "List Item Receiving"
      }
