...
const OrderKonsinyasiDetail = () => {
  ...
  const [nomorKonsi, setNomorKonsi] = useState([])
  const [loadingKonsi, setLoadingKonsi] = useState(false)
  ...
  const mergedData = productNeeds?.map((product) => {
    const delivery = orderDeliveries?.find(
      (delivery) => delivery.product_sku === product.sku // Sesuaikan key ID untuk penggabungan
    )
  ...
  console.log("productNeeds", productNeeds)
  ...
  const handleAssignWarehouse = () => {
    assignWarehouse(
      `/api/order-konsinyasi/assign-warehouse/${params.uid_lead}`
    ).then(({ error }) => {
      if (error) {
  ...
  const loadNomorKonsi = async (id) => {
    setLoadingKonsi(true)
    await axios
      .get("/api/general/order-konsi/" + id)
      .then((res) => {
        setNomorKonsi(res.data.data)
        setLoadingKonsi(false)
      })
      .catch(() => setLoadingKonsi(false))
  }

  useEffect(() => {
    if (orderDetail) {
      orderDetail?.notes && setNotes(orderDetail?.notes)
      loadNomorKonsi(orderDetail?.id)
    }
    // loadDetailOrderLead()
  }, [orderDetail])
  ...
  const onFinishSaveResi = (values) => {
    // if (!values?.resi) {
    //   return message.error("Nomor Resi Belum diisi")
    // }
    // split-delivery-order
    updateShippingInfo({
      url: "/api/order-konsinyasi/split-delivery-order",
      body: values,
    }).then(({ error }) => {
      if (error) {
  ...
  return (
    <Layout
      ...
      rightContent={rightContent}
      href="/order/order-konsinyasi"
      lastItemLabel={orderDetail?.order_number || params?.uid_lead}
    >
      <Steps
        size="small"
      ...
      <RenderIf isTrue={inArray(orderDetail?.status, ["1"])}>
        <div>
          ...
          {/* <Card title={"Nomor Order Konsinyasi"} className={"mt-4 mb-4"}>
            <Table
              columns={[...nomorOrderKonsiListColumns]}
              dataSource={nomorKonsi}
              rowKey={"id"}
              loading={loadingKonsi}
              pagination={false}
            />
          </Card> */}
...
export default OrderKonsinyasiDetail
