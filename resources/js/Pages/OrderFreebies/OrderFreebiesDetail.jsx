...
const OrderFreebiesDetail = () => {
  ...
  const handleAssignWarehouse = () => {
    assignWarehouse(`/api/freebies/assign-warehouse/${params.uid_lead}`).then(
      ({ error }) => {
        if (error) {
  ...
  const handleChangeKurir = (courier) => {
    if (orderDetail?.courier !== courier) {
      changePICWarehouse({
        url: `/api/freebies/change-courier`,
        body: {
          courier,
  ...
  const handleVerifyBilling = (value, status) => {
    ...
    billingOrderVerification({
      url: `/api/freebies/billing/verify`,
      body: { status, ...value },
    }).then(({ error }) => {
      if (error) {
  ...
  const onFinishSaveResi = (values) => {
    ...
    updateShippingInfo({
      url: "/api/freebies/split-delivery-order",
      body: values,
    }).then(({ error }) => {
      if (error) {
  ...
  const handleInsertInvoice = (id, multiple = false, invoice_id = null) => {
    if (multiple) {
      insertInvoice({
        url: `/api/freebies/product-need/invoice`,
        body: {
          is_invoice: 1,
      ...
      insertInvoice({
        url: `/api/freebies/product-need/invoice/${id}`,
        body: {
          is_invoice: 1,
          invoice_id,
        },
  ...
  const handleCancelInvoiceDelivery = (invoice_id) => {
    ...
    cancelInvoiceDelivery(`/api/freebies/delivery/cancel/${invoice_id}`).then(
      ({ error }) => {
        if (error) {
  ...
  const handleExportContent = () => {
    ...
    axios
      .post(`/api/freebies/export/detail/${params.uid_lead}`)
      .then((res) => {
        const { data } = res.data
        window.open(data)
  ...
  if (detailSalesOrderLoading || detailSalesOrderFetching) {
    return (
      <Layout title="Detail" rightContent={rightContent} href="/order/freebies">
        <LoadingFallback />
      </Layout>
    )
  }
...
export default OrderFreebiesDetail
