  ...
  return (
    <Menu
      onClick={({ key }) => {
        switch (key) {
          case "detail":
            navigate(`/order/freebies/detail/${value}`)
            break
          case "detail_new_tab":
            window.open(`/order/freebies/detail/${value}`)
            break
          case "update":
            navigate(`/order/freebies/form/${value}`)
            break
          case "cancel":
            return axios.get(`/api/freebies/cancel/${value}`).then((res) => {
      ...
      return (
        ...
        <div className="flex flex-row items-center justify-between gap-4">
          {inArray(status, ["Draft", "New"]) && (
            <Popconfirm
              title={"Apakah anda yakin ingin mengapus data ini?"}
              onConfirm={() => {
                return axios.get(`/api/freebies/cancel/${uid}`).then((res) => {
                  message.success("Data order berhasil dicancel!")
                  window.location.reload()
                })
              }}
          ...
          {inArray(status, ["Draft", "New"]) && (
            <Link to={`/order/freebies/form/${uid}`}>
              <Button icon={<EditOutlined />} />
            </Link>
          )}

          <Link to={`/order/freebies/detail/${uid}`}>
