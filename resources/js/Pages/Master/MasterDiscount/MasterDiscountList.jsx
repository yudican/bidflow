...
import { discountListColumn } from "./config"

const MasterDiscountList = () => {
  ...
  const loadData = (
    url = "/api/master/master-discount",
    perpage = 10,
  ...
  const handleChange = (page, pageSize = 10) => {
    loadData(`/api/master/master-discount/?page=${page}`, pageSize, {
      search,
      page,
      ...filterData,
    })
  ...
  const handleChangeSearch = () => {
    setIsSearch(true)
    loadData(`/api/master/master-discount`, 10, { search })
  }

  const handleFilter = (data) => {
    setFilterData(data)
    loadData(`/api/master/master-discount`, 10, data)
  }

  const deletePaymentTermId = (discount_id) => {
    axios
      .post(`/api/master/master-discount/delete/${discount_id}`, {
        _method: "DELETE",
      })
      .then((res) => {
        toast.success("Data Master Discount berhasil dihapus")
        loadData()
      })
      .catch((err) => {
        toast.error("Data Master Discount gagal dihapus")
      })
  }

  const updateStatus = (record) => {
    axios
      .post(`/api/master/master-discount/save/${record.id}`, record)
      .then((res) => {
  ...
  return (
    <Layout rightContent={rightContent} title="List Discount">
      <div className="card">
...
export default MasterDiscountList
