...
const BrandList = () => {
      ...
      .then((res) => {
        ...
        const newData = data.map((item) => {
          return {
            ...
            phone: item?.phone,
            status: item?.status,
            customerlist: JSON.stringify(item.brand_customer_support),
            logo: null,
  ...
  const handleChange = (page, pageSize = 10) => {
    loadData(`/api/master/brand/?page=${page}`, pageSize, {
      search,
      page,
      ...filterData,
    })
  ...
  const handleChangeSearch = () => {
    setIsSearch(true)
    loadData(`/api/master/brand`, 10, { search })
  }

  const handleFilter = (data) => {
    setFilterData(data)
    loadData(`/api/master/brand`, 10, data)
  }

  const deleteBrand = (brand_id) => {
    axios
      .post(`/api/master/brand/delete/${brand_id}`, { _method: "DELETE" })
      .then(({ data }) => {
        const { message } = data
        toast.success(message || "Brand berhasil dihapus")
        loadData()
      })
      .catch((err) => {
        const { message } = err.response.data
        toast.error(message || "Brand gagal dihapus")
      })
  }

  const updateStatus = (record) => {
    axios
      .post(`/api/master/brand/save/${record.id}`, record)
      .then((res) => {
  ...
  return (
    <Layout rightContent={rightContent} title="List Brand">
      <div className="card">
