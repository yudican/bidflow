...
import { checkbookListColumn } from "./config"
...
const CheckbookList = () => {
  ...
  const syncCheckbook = (url = "/api/master/checkbook/sync") => {
    setLoading(true)
    axios
      .post(url, {})
      .then((res) => {
  ...
  const handleChange = (page, pageSize = 10) => {
    loadData(`/api/master/checkbook/?page=${page}`, pageSize, {
      search,
      page,
      ...filterData,
    })
  ...
  const handleChangeSearch = () => {
    setIsSearch(true)
    loadData(`/api/master/checkbook`, 10, { search })
  }

  const handleFilter = (data) => {
    setFilterData(data)
    loadData(`/api/master/checkbook`, 10, data)
  }

  const deleteCheckbookId = (payment_term_id) => {
    axios
      .post(`/api/master/checkbook/delete/${payment_term_id}`, {
        _method: "DELETE",
      })
      .then((res) => {
        toast.success("Data Checkbook berhasil dihapus")
        loadData()
      })
      .catch((err) => {
        toast.error("Data Checkbook gagal dihapus")
      })
  }
  ...
  const rightContent = (
    <div className="flex justify-between items-center">
      ...
      >
        <SyncOutlined />
        <span className="ml-2">Sync Checkbook</span>
      </button>
      {show && (
        <button
  ...
  return (
    <Layout rightContent={rightContent} title="List Master Checkbook">
      <div className="card">
                      ...
                      overlay={
                        <Menu itemIcon={<RightOutlined />}>
                          ...
                          <Popconfirm
                            ...
                            onConfirm={() => deleteCheckbookId(text.id)}
                            // onCancel={cancel}
                            okText="Ya, Hapus"
                            cancelText="Batal"
                          >
