...
import { sourceCaseListColumn } from "./config"
...
const SourceCaseList = () => {
  ...
  const handleChange = (page, pageSize = 10) => {
    loadData(`/api/master/source-case/?page=${page}`, pageSize, {
      search,
      page,
      ...filterData,
    })
  ...
  const handleChangeSearch = () => {
    setIsSearch(true)
    loadData(`/api/master/source-case`, 10, { search })
  }

  const handleFilter = (data) => {
    setFilterData(data)
    loadData(`/api/master/source-case`, 10, data)
  }

  const deleteTypeCase = (source_case_id) => {
    axios
      .post(`/api/master/source-case/delete/${source_case_id}`, {
        _method: "DELETE",
      })
      .then((res) => {
        toast.success("Source Case berhasil dihapus")
        loadData()
      })
      .catch((err) => {
        toast.error("Source Case gagal dihapus")
      })
  }

  const updateStatus = (record) => {
    axios
      .post(`/api/master/source-case/save/${record.id}`, record)
      .then((res) => {
        toast.success("Source Case berhasil diupdate")
        loadData()
      })
      .catch((err) => {
        toast.error("Source Case gagal Di update")
      })
  }
  ...
  return (
    <Layout rightContent={rightContent} title="List Source Case">
      <div className="card">
