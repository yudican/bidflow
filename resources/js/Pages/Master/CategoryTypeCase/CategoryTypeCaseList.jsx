...
import { typeCaseListColumn } from "./config"

const CategoryTypeCaseList = () => {
  ...
  const loadData = (
    url = "/api/master/category-type-case",
    perpage = 10,
  ...
  const handleChange = (page, pageSize = 10) => {
    loadData(`/api/master/category-type-case/?page=${page}`, pageSize, {
      search,
      page,
      ...filterData,
    })
  ...
  const handleChangeSearch = () => {
    setIsSearch(true)
    loadData(`/api/master/category-type-case`, 10, { search })
  }

  const handleFilter = (data) => {
    setFilterData(data)
    loadData(`/api/master/category-type-case`, 10, data)
  }

  const deleteTypeCase = (type_case) => {
    axios
      .post(`/api/master/category-type-case/delete/${type_case}`, {
        _method: "DELETE",
      })
      .then((res) => {
        toast.success("Category Type Case berhasil dihapus")
        loadData()
      })
      .catch((err) => {
        toast.error("Type Case gagal dihapus")
      })
  }

  const updateStatus = (record) => {
    axios
      .post(`/api/master/category-type-case/save/${record.id}`, record)
      .then((res) => {
  ...
  return (
    <Layout rightContent={rightContent} title="List Category Type Case">
      <div className="card">
...
export default CategoryTypeCaseList
