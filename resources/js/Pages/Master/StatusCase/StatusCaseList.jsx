...
import { statusCaseListColumn } from "./config"
...
const StatusCaseList = () => {
  ...
  const handleChange = (page, pageSize = 10) => {
    loadData(`/api/master/status-case/?page=${page}`, pageSize, {
      search,
      page,
      ...filterData,
    })
  ...
  const handleChangeSearch = () => {
    setIsSearch(true)
    loadData(`/api/master/status-case`, 10, { search })
  }

  const handleFilter = (data) => {
    setFilterData(data)
    loadData(`/api/master/status-case`, 10, data)
  }

  const deleteTypeCase = (status_case_id) => {
    axios
      .post(`/api/master/status-case/delete/${status_case_id}`, {
        _method: "DELETE",
      })
      .then((res) => {
        toast.success("Status Case berhasil dihapus")
        loadData()
      })
      .catch((err) => {
        toast.error("Status Case gagal dihapus")
      })
  }

  const updateStatus = (record) => {
    axios
      .post(`/api/master/status-case/save/${record.id}`, record)
      .then((res) => {
  ...
  return (
    <Layout rightContent={rightContent} title="List Status Case">
      <div className="card">
