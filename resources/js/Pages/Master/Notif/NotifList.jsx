...
import { notifListColumn } from "./config"
...
const NotifList = () => {
  ...
  const handleChange = (page, pageSize = 10) => {
    loadData(`/api/master/notification/?page=${page}`, pageSize, {
      search,
      page,
      ...filterData,
    })
  ...
  const handleChangeSearch = () => {
    setIsSearch(true)
    loadData(`/api/master/notification`, 10, { search })
  }

  const handleFilter = (data) => {
    setFilterData(data)
    loadData(`/api/master/notification`, 10, data)
  }

  const deleteNotif = (notif_id) => {
    axios
      .post(`/api/master/notification/delete/${notif_id}`, {
        _method: "DELETE",
      })
      .then((res) => {
        toast.success(" Data Notif Alert berhasil dihapus")
        loadData()
      })
      .catch((err) => {
        toast.error(" Data Notif Alert gagal dihapus")
      })
  }

  const updateStatus = (record) => {
    axios
      .post(`/api/master/notification/save/${record.id}`, record)
      .then((res) => {
  ...
  return (
    <Layout rightContent={rightContent} title="List Notif Alert">
      <div className="card">
