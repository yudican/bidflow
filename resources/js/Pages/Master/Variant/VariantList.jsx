...
import { variantListColumn } from "./config"
...
const VariantList = () => {
  ...
  const handleChange = (page, pageSize = 10) => {
    loadData(`/api/master/variant/?page=${page}`, pageSize, {
      search,
      page,
      ...filterData,
    })
  ...
  const handleChangeSearch = () => {
    setIsSearch(true)
    loadData(`/api/master/variant`, 10, { search })
  }

  const handleFilter = (data) => {
    setFilterData(data)
    loadData(`/api/master/variant`, 10, data)
  }

  const deletePackage = (variant_id) => {
    axios
      .post(`/api/master/variant/delete/${variant_id}`, { _method: "DELETE" })
      .then((res) => {
  ...
  const updateStatus = (record) => {
    axios
      .post(`/api/master/variant/save/${record.id}`, record)
      .then((res) => {
  ...
  return (
    <Layout rightContent={rightContent} title="List Variant">
      <div className="card">
