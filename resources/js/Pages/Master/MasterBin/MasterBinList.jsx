...
const MasterBinList = () => {
  ...
  const handleChange = (page, pageSize = 10) => {
    loadData(`/api/master/master-bin/?page=${page}`, pageSize, {
      search,
      page,
      ...filterData,
    })
  ...
  const handleChangeSearch = () => {
    setIsSearch(true)
    loadData(`/api/master/master-bin`, 10, { search })
  }

  const handleFilter = (data) => {
    setFilterData(data)
    loadData(`/api/master/master-bin`, 10, data)
  }

  const deleteMasterBin = (warehouse_id) => {
    axios
      .post(`/api/master/master-bin/delete/${warehouse_id}`, {
        _method: "DELETE",
      })
      .then((res) => {
        toast.success("Data Master BIN berhasil dihapus")
        loadData()
      })
      .catch((err) => {
        toast.error("Data Master BIN gagal dihapus")
      })
  }

  const updateStatus = (record) => {
    axios
      .post(`/api/master/master-bin/save/${record.id}`, record)
      .then((res) => {
  ...
  return (
    <Layout rightContent={rightContent} title="List BIN">
      <div className="card">
                    ...
                    overlay={
                      <Menu itemIcon={<RightOutlined />}>
                        ...
                        <Popconfirm
                          ...
                          onConfirm={() => deleteMasterBin(text.id)}
                          // onCancel={cancel}
                          okText="Ya, Hapus"
                          cancelText="Batal"
                        >
