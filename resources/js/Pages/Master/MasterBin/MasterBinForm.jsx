...
import { formatPhone, inArray, validatePhoneNumber } from "../../../helpers"
import "../../../index.css"
...
const MasterBinForm = () => {
  ...
  const loadDetailBrand = () => {
    axios.get(`/api/master/master-bin/${master_bin_id}`).then((res) => {
      const { data } = res.data
      setDetail(data)
      ...
      form.setFieldsValue({ ...data, telepon: formatPhone(data?.telepon) })
    })
  }
  ...
  const onFinish = (values) => {
    ...
    const url = master_bin_id
      ? `/api/master/master-bin/save/${master_bin_id}`
      : "/api/master/master-bin/save"

    axios
      .post(url, formData)
      .then((res) => {
        toast.success("Data Master BIN berhasil disimpan", {
          position: toast.POSITION.TOP_RIGHT,
        })
        setLoadingSubmit(false)
        return navigate("/master/bin")
      })
      .catch((err) => {
            ...
            <div className="col-md-6">
              <Form.Item
                ...
                rules={[
                  {
                    required: true,
                    message: "Silakan masukkan BIN Name!",
                  },
                ]}
              >
                <Input placeholder="Ketik BIN Name" />
              </Form.Item>
            ...
            <div className="col-md-6">
              <Form.Item
                ...
                rules={[
                  {
                    required: true,
                    message: "Silakan masukkan BIN Type!",
                  },
                ]}
              >
                <Input placeholder="Ketik BIN Type " />
              </Form.Item>
