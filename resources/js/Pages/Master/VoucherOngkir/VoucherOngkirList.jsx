...
const VoucherList = () => {
  ...
  const handleChange = (page, pageSize = 10) => {
    loadData(`/api/master/voucher-ongkir/?page=${page}`, pageSize, {
      search,
      page,
      ...filterData,
    })
  ...
  const handleChangeSearch = () => {
    setIsSearch(true)
    loadData(`/api/master/voucher-ongkir`, 10, { search })
  }

  const handleFilter = (data) => {
    setFilterData(data)
    loadData(`/api/master/voucher-ongkir`, 10, data)
  }

  const deleteVoucher = (banner_id) => {
    axios
      .post(`/api/master/voucher-ongkir/delete/${banner_id}`, {
        _method: "DELETE",
      })
      .then((res) => {
  ...
  const updateStatus = (record) => {
    axios
      .post(`/api/master/voucher-ongkir/save/${record.id}`, record)
      .then((res) => {
