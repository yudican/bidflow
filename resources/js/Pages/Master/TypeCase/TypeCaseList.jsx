...
const TypeCaseList = () => {
  ...
  const handleChange = (page, pageSize = 10) => {
    loadData(`/api/master/type-case/?page=${page}`, pageSize, {
      search,
      page,
      ...filterData,
    })
  ...
  const handleChangeSearch = () => {
    setIsSearch(true)
    loadData(`/api/master/type-case`, 10, { search })
  }

  const handleFilter = (data) => {
    setFilterData(data)
    loadData(`/api/master/type-case`, 10, data)
  }

  const deleteTypeCase = (type_case) => {
    axios
      .post(`/api/master/type-case/delete/${type_case}`, {
        _method: "DELETE",
      })
      .then((res) => {
        toast.success("Type Case berhasil dihapus")
        loadData()
      })
      .catch((err) => {
  ...
  const updateStatus = (record) => {
    axios
      .post(`/api/master/type-case/save/${record.id}`, record)
      .then((res) => {
  ...
  return (
    <Layout rightContent={rightContent} title="List Type Case">
      <div className="card">
