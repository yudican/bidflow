...
import { salesChannelListColumn } from "./config"
...
const SalesChannelList = () => {
  ...
  const handleChange = (page, pageSize = 10) => {
    loadData(`/api/master/sales-channel/?page=${page}`, pageSize, {
      search,
      page,
      ...filterData,
    })
  ...
  const handleChangeSearch = () => {
    setIsSearch(true)
    loadData(`/api/master/sales-channel`, 10, { search })
  }

  const handleFilter = (data) => {
    setFilterData(data)
    loadData(`/api/master/sales-channel`, 10, data)
  }

  const deletePackage = (variant_id) => {
    axios
      .post(`/api/master/sales-channel/delete/${variant_id}`, {
        _method: "DELETE",
      })
      .then((res) => {
  ...
  const rightContent = (
    <div className="flex justify-between items-center">
      <FilterModal handleOk={handleFilter} warehouses={warehouses} />
      {show && (
        <button
  ...
  return (
    <Layout rightContent={rightContent} title="List Sales Channel">
      <div className="card">
