...
const PaymentTermList = () => {
  ...
  const syncData = (url = "/api/master/payment-term/sync") => {
    setLoading(true)
    axios
      .post(url, {})
      .then((res) => {
        toast.success("Sync data Payment term berhasil")
        loadData()
      })
      .catch((e) => {
        toast.success("Sync data Payment term gagal")
        loadData()
        setLoading(false)
      })
  }
  ...
  const handleChange = (page, pageSize = 10) => {
    loadData(`/api/master/payment-term/?page=${page}`, pageSize, {
      search,
      page,
      ...filterData,
    })
  ...
  const handleChangeSearch = () => {
    setIsSearch(true)
    loadData(`/api/master/payment-term`, 10, { search })
  }

  const handleFilter = (data) => {
    setFilterData(data)
    loadData(`/api/master/payment-term`, 10, data)
  }

  const deletePaymentTermId = (payment_term_id) => {
    axios
      .post(`/api/master/payment-term/delete/${payment_term_id}`, {
        _method: "DELETE",
      })
      .then((res) => {
      ...
      .catch((err) => {
        toast.error("Payment Term gagal dihapus")
      })
  }

  const updateStatus = (record) => {
    axios
      .post(`/api/master/payment-term/save/${record.id}`, record)
      .then((res) => {
  ...
  return (
    <Layout rightContent={rightContent} title="List Payment Term">
      <div className="card">
