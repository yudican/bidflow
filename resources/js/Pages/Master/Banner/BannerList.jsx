...
import { bannerListColumn } from "./config"
...
const BannerList = () => {
  ...
  const handleChange = (page, pageSize = 10) => {
    loadData(`/api/master/banner/?page=${page}`, pageSize, {
      search,
      page,
      ...filterData,
    })
  ...
  const handleChangeSearch = () => {
    setIsSearch(true)
    loadData(`/api/master/banner`, 10, { search })
  }

  const handleFilter = (data) => {
    setFilterData(data)
    loadData(`/api/master/banner`, 10, data)
  }

  const deleteBanner = (banner_id) => {
    axios
      .post(`/api/master/banner/delete/${banner_id}`, { _method: "DELETE" })
      .then((res) => {
  ...
  const updateStatus = (record) => {
    axios
      .post(`/api/master/banner/save/${record.id}`, record)
      .then((res) => {
  ...
  return (
    <Layout rightContent={rightContent} title="List Banner">
      <div className="card">
