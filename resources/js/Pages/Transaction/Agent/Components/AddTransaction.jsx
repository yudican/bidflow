...
import { useGetTopQuery } from "../../../../configs/Redux/Services/generalServices"
import {
  formatNumber,
  getItem,
  inArray,
...
const AddTransaction = ({ refetch }) => {
  ...
  const loadProducts = () => {
    axios.get("/api/master/products/agent-portal").then((res) => {
      if (res) {
  ...
  const handleGetDiscount = () => {
    return axios
      ...
      .then((res) => {
        ...
        setDiscounts(
          data
            ?.filter((item) => inArray("agent-portal", item.sales_channels))
            .map((row) => {
  ...
  useEffect(() => {
    loadProducts()
    if (inArray(role, ["agent", "subagent"])) {
      handleSearchUser(userData?.telepon)
    }
    handleGetDiscount()
  }, [])
    ...
    return axios
      ...
      .then((res) => {
        ...
        if (data) {
          ...
          const addressDetail = address.map((value) => {
            return {
              value: value.alamat,
              label: value.alamat_detail,
              telepon: value.telepon,
  ...
  const handleSubmit = (values) => {
    ...
    axios
      .post("/api/sales-order/agent/new-order", data)
      .then((res) => {
    ...
    return (
      <button
        onClick={(e) => {
          ...
          form.setFieldValue("shipping_method_id", item.logistic_name)
        }}
          ...
          <div key={"search"} className="flex justify-between my-2">
            ...
            <div className="flex-1">
              <Button
                ...
                disabled={products.every((product) => product.qty < 1)}
                // loading={loading}
                // onClick={handleOk}
              >
                Proses Transaksi
            ...
            >
              ...
              >
                <Input
                  ...
                  disabled={inArray(getItem("role"), ["agent", "subagent"])}
                />
              </Form.Item>
              <Form.Item
              ...
              >
                {loadingUserData ? (
                  ...
                  <Input
                    placeholder="Ketik nama lengkap Customer.."
                    disabled={inArray(getItem("role"), ["agent", "subagent"])}
                  />
                )}
              </Form.Item>
              ...
              {newAddress && adressUser?.length >= 0 ? (
                <Form.Item
                  label="Kecamatan"
                  name="kecamatan_id"
                  rules={[
              ...
              >
                <Select placeholder={"Pilih Shipping Method"}>
                  {shippingList &&
                    shippingList.map((top) => (
              ...
              <Form.Item
                ...
                rules={[
                  {
                    required: true,
                    message: "Silakan masukkan Payment Term!",
                  },
                ]}
              >
                <Select placeholder={"Pilih Payment Term"}>
                  {termOfPayments &&
