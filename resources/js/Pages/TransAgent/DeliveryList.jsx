...
import { DeliveryListColumn } from "./config"
...
const DeliveryList = () => {
  ...
  const loadContact = (url = "/api/deliveryAgent", perpage = 10) => {
    setLoading(true)
      ...
      <Menu
        onClick={({ key }) => {
          switch (key) {
            ...
            case "diterima":
              axios
                .post(`/api/trans-agent/product-receive/${value}`, {
                  value: value,
                })
                .then((res) => {
  ...
  return (
    <Layout rightContent={rightContent} title="Transaction Agent - Pengiriman">
      <div className="card">
        <div className="card-body">
          <Table
            dataSource={contacts}
            columns={[...DeliveryListColumn, Aksi]}
            loading={loading}
            pagination={false}
            rowKey="id"
