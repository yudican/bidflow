...
const StockSystemCalculated = () => {
            ...
            (cust.subs || []).forEach((sub) => {
                ...
                (sub.details || []).forEach((item) => {
                    exportRows.push({
                        ...
                        "Avg Daily Sales (3 bln)": item.avg_daily_sales || 0,
                        "Days on Hand": item.days_on_hand || 0,
                        "Perkiraan Habis Stok": item.runout_date || "-",
                    });
                });
            });
        });
    ...
    const opnameDate =
        stockData?.[0]?.subs?.[0]?.details?.[0]?.opname_date || null;
    ...
    const detailColumns = [
        ...
        // { title: "Stock Awal (Data by Accurate)", dataIndex: "stock_opname", align: "right", render: () => 0 },
        {
            title: (
                <span>
                    Stock Awal (Data by Stock Opname){" "}
                    {opnameDate && (
                        <Tooltip
                            title={`Last sync date: ${dayjs(opnameDate).format("DD-MM-YYYY")}`}
                        >
                            <QuestionCircleOutlined
                                style={{ color: "#1890ff", marginLeft: 6, cursor: "pointer" }}
                            />
                        </Tooltip>
                    )}
                </span>
            ),
            dataIndex: "stock_opname",
            align: "right",
            render: (val) => Intl.NumberFormat("id-ID").format(val || 0),
        },
        ...
        {
            title: "Avg. Sales Out/Days",
            dataIndex: "avg_daily_sales",
            align: "right",
            render: (val) => Intl.NumberFormat("id-ID", { maximumFractionDigits: 1 }).format(val || 0),
        },
        {
            title: "Avg. Sales Out/Month",
            dataIndex: "avg_month_sales",
            align: "right",
            render: (val) => Intl.NumberFormat("id-ID", { maximumFractionDigits: 1 }).format(val || 0),
        },
        {
            title: "Total Sales/3 Month",
            dataIndex: "avg_daily_sales",
            align: "right",
            render: (_, record) => {
                const qtyOrder = record.qty_order || 0
                const totalDays = record.total_days || 0
                return `${qtyOrder} / ${totalDays}`
            },
        },
        {
            title: "DoH (Days on Hand)",
            dataIndex: "days_on_hand",
            align: "right",
            render: (val) => Intl.NumberFormat("id-ID", { maximumFractionDigits: 1 }).format(val || 0),
        },
        {
            title: "Perkiraan Habis Stok",
            dataIndex: "runout_date",
            align: "center",
...
export default StockSystemCalculated;
