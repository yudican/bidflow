...
const OrderLeadList = () => {
  ...
  const [paramUrl, setParamUrl] = useState("/api/order-lead")
  const {
    data: salesOrderData,
  ...
  const handleChange = (page, pageSize = 10) => {
    setCurrentPage(page)
    loadOrder(`/api/order-lead`, pageSize, {
      search,
      page,
  ...
  const handleChangeSearch = () => {
    setIsSearch(true)
    loadOrder(`/api/order-lead`, 10, { search })
  }

  const handleFilter = (data) => {
    setFilterData(data)
    loadOrder(`/api/order-lead`, 10, data)
  }
  ...
  const handleSubmitGp = (value) => {
    ...
    submitSiToGP({
      url: `/api/order/order-lead/submit`,
      body: {
        ids: selectedRowKeys,
        type: "order-lead",
        ...value,
  ...
  const rightContent = (
    <div className="flex justify-between items-center">
      ...
      {show && (
        <button
          onClick={() => navigate("/order/order-lead/form")}
        ...
        >
          <PlusOutlined />
          <span className="ml-3">Tambah Order</span>
        </button>
      )}
    </div>
  )
