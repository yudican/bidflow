...
import { contactGroupListColumn } from "./config"
...
const ContactGroupList = () => {
  ...
  const handleChange = (page, pageSize = 10) => {
    loadData(`/api/contact-group/?page=${page}`, pageSize, {
      search,
      page,
      ...filterData,
    })
  ...
  const handleChangeSearch = () => {
    setIsSearch(true)
    loadData(`/api/contact-group`, 10, { search })
  }

  const handleFilter = (data) => {
    setFilterData(data)
    loadData(`/api/contact-group`, 10, data)
  }

  const deletePaymentTermId = (group_id) => {
    axios
      .post(`/api/contact-group/delete/${group_id}`, {
        _method: "DELETE",
      })
      .then((res) => {
      ...
      .catch((err) => {
        toast.error("Group gagal dihapus")
      })
  }
  ...
  return (
    <Layout rightContent={rightContent} title="List Contact Group">
      <div className="card">
