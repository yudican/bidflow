          ...
          <div className="card">
            ...
            <div className="card-body">
              ...
              <Table
                ...
                loading={productNeedsLoading}
                pagination={false}
                rowKey="id"
                summary={(productNeed) => {
                  if (productNeed && productNeed.length > 0) {
                    const price = productNeed.reduce(
                      (acc, curr) => parseInt(acc) + parseInt(curr.price),
                      0
                    )
                    const subtotal = productNeed.reduce(
                      (acc, curr) =>
                        parseInt(acc) + parseInt(curr.price * curr.qty),
                      0
                    )

                    const discount_amount = productNeed.reduce(
                      (acc, curr) =>
                        parseInt(acc) + parseInt(curr.discount_amount || 0),
                      0
                    )

                    const ppn = productNeed.reduce(
                      (acc, curr) =>
                        parseInt(acc) + parseInt(curr.tax_amount || 0),
                      0
                    )
                    const dpp = price - discount_amount
                    const subtotal_amount = subtotal - discount_amount
                    const total =
                      subtotal_amount +
                      ppn +
                      orderDetail?.kode_unik +
                      orderDetail?.ongkir
                    return (
                      <Table.Summary>
                        <Table.Summary.Row>
                      ...
                      return (
                        <Table.Summary>
                          ...
                          <Table.Summary.Row>
                            ...
                            <Table.Summary.Cell align="left" colSpan={1}>
                              <strong>
                                Rp. {formatNumber(discount_amount)}
                              </strong>
                            </Table.Summary.Cell>
                            <Table.Summary.Cell />
                          </Table.Summary.Row>
                ...
                dataSource={
                  ...
                  orderDeliveries.map((item) => {
                    return {
                      ...
                      qty_delivered: item.qty_delivery,
                    }
                  })
                }
                        ...
                        overlay={
                          <Menu
                            onClick={({ key }) => {
                              if (key === "print") {
                                return window.open(
                                  `/print/sj/${record.uid_lead}/${record.uid_invoice}`
                                )
                              }
                              if (key === "print_si") {
                                return window.open(
          ...
          <div className="card">
            ...
            <div className="card-body">
              <Table
                ...
                loading={loadingInsertInvoice || orderDeliveryLoading}
                pagination={false}
                rowKey="id"
                summary={(currentData) => {
                  if (currentData && currentData.length > 0) {
                    ...
                    const ppn = currentData.reduce(
                      (acc, curr) =>
                        parseInt(acc) + parseInt(curr.tax_invoiced || 0),
                      0
                    )
