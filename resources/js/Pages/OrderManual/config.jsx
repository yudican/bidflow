...
import { Button, Menu, Steps, Tag, Tooltip, message, Popconfirm } from "antd"
      ...
      return [
        {
          ...
          children: [
            ...
            {
              label: "Sales Order New",
              key: "detail_sales_order",
              icon: <EyeOutlined />,
            },
          ],
        },
      ...
      onClick={({ key }) => {
        ...
        switch (key) {
          ...
          case "detail_new_tab":
            ...
            break
          case "detail_sales_order":
            window.open(`/order/sales-order/detail/${value}`)
            break
          case "update":
    ...
    render: (item) => {
      ...
      return (
        ...
        <div className="flex flex-row items-center justify-between gap-4">
          ...
          {inArray(status, ["Draft", "New"]) &&
            inArray(item.type, ["manual", "freebies"]) && (
              <Popconfirm
          ...
          {inArray(status, ["Draft", "New"]) &&
            inArray(item.type, ["manual", "freebies"]) && (
              <Link to={`/order/order-manual/form/${uid}`}>
                <Button icon={<EditOutlined />} />
              </Link>
            )}

          <Link to={`/order/order-${item.type}/detail/${uid}`}>
