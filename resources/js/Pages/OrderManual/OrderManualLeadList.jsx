...
const OrderManualLeadList = ({ type }) => {
  ...
  const { data: channelList, isLoading: loadingSalesChannel } =
    useGetSalesChannelQuery("manual")
  ...
  const menu = (
    <Menu>
      ...
      {type == "manual" && (
        <Menu.Item>
          <ModalOrderExport
            formData={{ ...filterData, user_id: userData?.id, type: "manual" }}
            type="manual"
          />
        </Menu.Item>
      )}
  ...
  return (
    <Layout rightContent={rightContent} title={`List Order ${type}`}>
      <div className="card">
