...
class LivewireDatatable extends Component
{
              ...
              foreach ($this->getColumnField($i) as $column) {
                $query->when(is_array($column), function ($query) use ($search, $column) {
                  foreach ($column as $col) {
                    $query->orWhereRaw('LOWER(' . $this->prefix . $col . ') like ?', '%' . strtolower($search) . '%');
                  }
                }, function ($query) use ($search, $column) {
                  $query->orWhereRaw('LOWER(' . $this->prefix . $column . ') like ?', '%' . strtolower($search) . '%');
                });
              }
            });
          });
            ...
            if ($this->columnIsRelation($this->freshColumns[$index])) {
              ...
              $query->orWhere(function ($query) use ($index, $value) {
                foreach ($this->getColumnField($index) as $column) {
                  ...
                  $query->orWhereRaw('LOWER(' . $this->prefix . $column . ') like ?', [strtolower("%$value%")]);
                }
              });
            }
          }
  ...
  public function toggleSelectAll()
  {
    $query = $this->getQuery();
    $selected = [];
    if (count($this->selected) > 0 && count($this->selected) == $query->count()) {
      $this->selected = $selected;
    } else {
      foreach ($query->pluck('id')->toArray() as $key => $value) {
        $selected[$value] = $value;
      }
      $this->selected = $selected;
    }

    $this->emit('getSelected', ['data' => $selected]);
    $this->forgetComputed();
  }
  public function getSelected($value)
  {
    $this->emit('getSelected', ['data' => $this->selected]);
  }

  public function setSelected($id, $type)
  {

    $selected = $this->selected;
    if (in_array($selected, [$id])) {
      $selected = array_diff($selected, [$id]);
    } else {
      $selected[] = $id;
    }
