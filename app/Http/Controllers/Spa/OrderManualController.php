...
use App\Imports\OrderManualImport;
...
class OrderManualController extends OrderInvoiceController
{
    ...
    public function listOrderLead(Request $request)
    {
        ...
        $query = OrderManual::query()
            ...
            ->where('order_manuals.type', $request->type ?? 'manual')
            ->select([
                'order_manuals.id',
    ...
    public function detailOrder($uid_lead)
    {
        $orderLead =  OrderManual::with([
            'productNeeds',
        ])->where('uid_lead', $uid_lead)->first();
    ...
    {
        $data = [
            ['label' => 'Corner', 'value' => 'corner', 'count' => OrderManual::whereType('manual')->where('company_id', $account_id)->whereHas('contactUser', function ($query) {
            ...
            ['label' => 'Agent Portal', 'value' => 'agent-portal', 'count' => OrderManual::whereType('manual')->where('company_id', $account_id)->whereHas('contactUser', function ($query) {
                $query->where('sales_channel', 'agent-portal');
            })->count()],
            ['label' => 'Distributor', 'value' => 'distributor', 'count' => OrderManual::whereType('manual')->where('company_id', $account_id)->whereHas('contactUser', function ($query) {
                $query->where('sales_channel', 'distributor');
            })->count()],
            ['label' => 'Super Agent', 'value' => 'super-agent', 'count' => OrderManual::whereType('manual')->where('company_id', $account_id)->whereHas('contactUser', function ($query) {
                $query->where('sales_channel', 'super-agent');
            })->count()],
            ['label' => 'Modern Store', 'value' => 'modern-store', 'count' => OrderManual::whereType('manual')->where('company_id', $account_id)->whereHas('contactUser', function ($query) {
                $query->where('sales_channel', 'modern-store');
            })->count()],
            ['label' => 'E-Store', 'value' => 'e-store', 'count' => OrderManual::whereType('manual')->where('company_id', $account_id)->whereHas('contactUser', function ($query) {
            ...
            if ($row) {
                ...
                foreach ($row->productNeeds as $key => $value) {
                    ...
                    if ($row->type == 'konsinyasi') {
                        ...
                        $this->updateStock($productNeed, $row->master_bin_id, $value['qty']);
                    }
                }
    ...
    public function saveOrderManual(Request $request)
    {
        try {
            ...
            $sales = $request->sales;
            $account_id = $user->company_id ?? $request->account_id;
            ...
            if (!$request->uid_lead) {
                $data['title'] = OrderManual::generateOrderNumber();
                $data['order_number'] = OrderManual::generateOrderNumber();
                $data['invoice_number'] = OrderManual::generateInvoiceNumber();
                ...
                $userMs = User::where('id', $request->contact)->first(['id', 'sales_channel']);
                if ($userMs && $userMs->sales_channel == 'modern-store') {
                    $data['notes'] = 'Jam penerimaan barang = Senin- Sabtu di jam 08.00-16.00 WIB, (pengiriman barang dikirim ke bagian Receiving).';
                } else if ($userMs && $userMs?->role?->role_type == 'ms') {
                    $data['notes'] = 'Jam penerimaan barang = Senin- Sabtu di jam 08.00-16.00 WIB, (pengiriman barang dikirim ke bagian Receiving).';
                }
            }
    ...
    public function updateOrderManual(Request $request)
    {
        try {
            DB::beginTransaction();
    ...
    private function generateOrderNo($no = 1)
    {
        ...
        $nomor = 'SO/' . $year . '/2' . date('mdH') . rand(1121, 9999);
        // $rw = OrderManual::whereNotNull('order_number')->whereType('manual')->orderBy('id', 'desc')->orderBy('order_number', 'desc')->first(['order_number']);
        // if ($rw) {
    ...
    private function generateInvoiceNo()
    {
        ...
        // $nomor = 'SI/' . $year . '/2' . $username . '0001';
        $nomor = 'SI/' . $year . '/2' . date('mdH') . rand(1121, 9999);
    ...
    public function splitOrderDelivery(Request $request)
    {
        ...
        try {
            ...
            foreach ($products as $key => $value) {
                ...
                $productMaster = Product::find($variant->product_id);
                $master_stock = (int) getStock($productMaster->stock_warehouse, $orderManual->warehouse_id, $orderManual->company_id, $variant->product_id);
                if ($master_stock < $value['qty']) {
                    DB::rollBack();
                    return response()->json([
                        'status' => 'error',
                ...
                $this->updateStock($productNeed, $orderManual->warehouse_id, $value['qty']);
            }
            $dataLog = [
                'log_type' => '[fis-dev]order_manual',
            ...
            if ($productNeed) {
                ...
                if ($variant->is_bundling > 0) {
                    ...
                    foreach ($bundlings as $key => $bundling) {
                        ...
                        $master_stock = (int) getStock($product->stock_warehouse, $row->warehouse_id, $row->company_id, $bundling->product_id);
                    ...
                    $master_stock = (int) getStock($product->stock_warehouse, $row->warehouse_id, $row->company_id, $variant->product_id);
    ...
    public function updateOngkosKirim(Request $request, $uid_lead)
    {
        try {
            ...
            $row = OrderManual::where('uid_lead', $uid_lead)->first();
            if ($row) {
                $row->update(['ongkir' => $request->ongkir[0]]);
            }
    ...
    public function updateInvoicedDate(Request $request, $uid_lead)
    {
        $orderManual =  OrderManual::where('uid_lead', $uid_lead)->first();
        if ($orderManual) {
            $time = date(' H:i:s');
            $item = OrderDelivery::where('uid_lead', $uid_lead);
            $item->update(['invoice_date' => $request->invoice_date . $time]);
            $orderManual->update(['invoice_date' => $request->invoice_date]);
        }
        return response()->json([
            'status' => 'success',
        ]);
    ...
    public function import(Request $request)
    {
        try {
            ...
            $import = new OrderManualImport($request->contact);
