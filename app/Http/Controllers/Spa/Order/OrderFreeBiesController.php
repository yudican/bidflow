...
use App\Imports\OrderFreebiesImport;
...
class OrderFreeBiesController extends GpController
{
    ...
    public function listOrderLead(Request $request)
    {
        ...
        $query = OrderManual::query()
            ->where('order_manuals.type', 'freebies')
            ->select([
                'order_manuals.id',
            ...
            ->leftJoin('payment_terms', function ($join) use ($request) {
                ...
                if ($request->payment_term) {
                    if (is_array($request->payment_term)) {
                        $join->whereIn('payment_terms.id', $request->payment_term);
                    } else {
                        $join->whereIn('payment_terms.id', explode(',', $request->payment_term));
                    }
                }
            })
        ...
        if ($request->created_at) {
            if (is_array($request->created_at)) {
                ...
                $query->whereBetween('created_at', explode(',', $request->created_at));
            }
        }
        ...
        $query->with(
            'orderDelivery:id,qty_delivered,invoice_number,gp_submit_number,submit_klikpajak,is_invoice,uid_invoice,uid_lead',
        );
        ...
        try {
            ...
            if ($row) {
                ...
                try {
                    ...
                    setSetting('so_frebies_ethix_body', json_encode($curl_post_data));
            ...
            if ($billing) {
                ...
                if ($request->status == 1) {
                    foreach ($billing->orderProductBillings as $key => $value) {
                        if ($billing->orderManual->master_bin_id) {
                            MasterBinStock::create([
                                'master_bin_id' => $billing->orderManual->master_bin_id,
                                'product_variant_id' => $value->product_id,
    ...
    public function saveOrderManual(Request $request)
    {
        try {
            ...
            if (!$request->uid_lead) {
                ...
                $data['order_number'] = OrderManual::generateOrderNumber(3);
                $data['invoice_number'] = OrderManual::generateInvoiceNumber(3);
            ...
            if (is_array($productItems) && count($productItems) > 0) {
                if ($request->uid_lead) {
                    $productItemIds = array_column($productItems, 'id');
                    ...
                    // Delete product needs that are not in $productItems
    ...
    private function generateTitle($brand = 'FLIMTY', $role)
    {
        ...
        $titleFinal = 'SO/' . $brand . '/' . $role . '-' . $date . '/001';
        $rw = OrderManual::where('title', 'like', '%' . $title . '%')->whereNotNull('title')->whereType('freebies')->orderBy('id', 'desc')->first();
        if ($rw) {
            $awal = substr($rw->title, -3);
            $next =  '3' . sprintf("%03d", ((int)$awal + 1));
            $titleFinal = 'SO/' . $brand . '/' . $role . '-' . $date . '/' . $next;
        }
    ...
    private function generateOrderNo($no = 1)
    {
        ...
        $nomor = 'SO/' . $year . '/2' . date('mdH') . rand(1233, 9999);
                ...
                foreach ($bundlings as $key => $bundling) {
                    ...
                    foreach ($product_variants as $key => $variant) {
                        ...
                        saveLogStock([
                            ...
                            'description' => 'Transaction Product SO Freebies - ' . $trans->uid_lead,
                        ]);
                    }
    ...
    {
        //print_r($request);
        $file_name = 'convert/FIS-Order_Freebies-' . date('d-m-Y') . '.xlsx';
    ...
    public function updatePrice(Request $request)
    {
        try {
            ...
            $product = ProductNeed::find($id);

            $product->update(['price' => $price, 'price_type' => 'manual']);
    ...
    public function import(Request $request)
    {
        try {
            ...
            $import = new OrderFreebiesImport($request->contact);
